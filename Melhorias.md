...
- 🔲 **R3.2.B.3 pendente**: persistência de `_pair_positions` em tabela `robot_pair_positions` (in-memory perde estado em restart); `naked_leg` deveria emitir Pushover alert (hoje só log); robot_pair_signals_log para audit (hoje só structlog). Defer pra depois do smoke live validar fluxo.
- 🔲 **R3.3 UI `/pairs`** (1-2d): lista pares ativos + chart Z-score histórico.
...
- ✅ **Fase B.1 COMPLETA** (01/mai 16h): cutover end-to-end funcional. 17 containers up no Engine WSL2, DBs preservadas (28 cointegrated_pairs, 884 fintz tickers, alembic ts_0004 + 0023). Container → `host.docker.internal:8002` → `{"ok":true}`. Windows `localhost:8000` → `{"ok":true}` (WSL2 port forward auto). Smoke segunda 11h vai rodar contra esse stack. **Comandos Docker agora via** `wsl -d Ubuntu-22.04 -- docker ...` (PowerShell `docker compose` aponta pra Docker Desktop daemon que está vazio agora).
