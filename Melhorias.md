...
- ✅ **R3.2.B.3 COMPLETO** (01/mai): persistência infra + worker integration. Alembic 0024 + `PsycopgPairPositionsRepository` (9 unit tests). Worker `_evaluate_pairs` agora usa `positions_repo` direto (Protocol satisfeito sem adapter — repo.get() expõe a interface esperada). OPEN dispara `repo.upsert(pair_key, position, last_cl_ord_id=cl_a)`; CLOSE/STOP dispara `repo.delete(pair_key)`. Sobrevive restart NSSM/container. Pushover alert pra naked_leg ainda defer (somente structlog hoje).
- ✅ **R3.3 UI `/pairs`** stub read-only (commit `5e2afc0+`): `pairs.html` lista posições abertas (`robot_pair_positions`) + pares cointegrados ativos com cores semafóricas pra p-ADF. Routes `/api/v1/pairs/active` (LEFT JOIN com positions) + `/api/v1/pairs/positions`. Auto-refresh 30s. Z-score real-time não exposto ainda — vem em iteração futura.
