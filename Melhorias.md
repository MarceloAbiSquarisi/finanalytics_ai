...
- 🟡 **R3.2.B.3 parcial** (01/mai): persistência infra pronta — Alembic 0024 cria `robot_pair_positions(pair_key PK, position CHECK LONG_SPREAD/SHORT_SPREAD, opened_at, last_dispatch_cl_ord_id, updated_at)`. `PsycopgPairPositionsRepository` com `get/all/upsert/delete` (9 unit tests verde). Worker integration substituindo `_pair_positions` in-memory ainda pendente — defer p/ pós-smoke 2ª. Pushover alert pra naked_leg também defer.
