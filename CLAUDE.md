...
19. ~~Sprint Backend 21/abr — V1+V2+V3+V4+Z3+Z4~~ — **DONE 21/abr** — 3 commits (`83ae2c8`, `c4113ab`, `de57c44`):
    ...
    - **Pendências da Sprint Backend** (sub-itens abertos):
      - **C** `GRAFANA_SLACK_WEBHOOK` real ainda é placeholder em `contact-points.yml`. Alertas `severity=critical` (4 de 9) não chegam a um canal de fato. Setar via env var no compose quando workspace Slack pronto.
      - **D** SMTP não configurado — `default-internal` é email noop. Postpostado a sessão futura (Gmail App Password ou Mailgun).
      - **E** ~~`/api/v1/ml/metrics` retorna JSON, não Prometheus~~ — **EM PROGRESSO** (Sprint Fix Alerts 21/abr). Quando concluído, alertas de `ml_drift_count > 0` e `ml_snapshot_age_days > 2` poderão ser provisionados em `rules.yml`.
      - **F** Nenhum alert rule sobre falhas do `reconcile_loop` (scheduler). Falhas hoje só ficam em log; criar rule + métrica `scheduler_reconcile_errors_total` (precisa expor scheduler_worker em :9102 ou similar).
      - **H** profit_agent (Windows host) não foi reiniciado — V1 logs throttled aplicados em código mas não em runtime. Ativação manual: relaunch `.venv\Scripts\python.exe src\finanalytics_ai\workers\profit_agent.py`.
      - **I** Scheduler container roda imagem antiga; mudanças via `docker cp` perdidas em recreate. Próximo deploy: `docker compose up -d --build scheduler`.
