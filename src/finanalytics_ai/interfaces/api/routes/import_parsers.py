"""
Parsers de extratos e notas de corretagem — extraido de import_route.py
em 01/mai/2026 (sessao limpeza profunda).

Funcoes puras que recebem bytes do upload + filename e retornam dict
estruturado com trades/positions normalizados. Sem efeito colateral em
DB ou state — chamadores (routes) sao responsaveis por persistir.

Suporte:
  - _parse_nota_xp: Nota de negociacao B3 PDF
  - _parse_posicao_xp: Posicao Detalhada XP XLSX
  - _parse_extrato_btg_us: Account Statement BTG US PDF
  - _parse_csv: CSV generico de posicoes
  - _parse_extrato_btg_br: Extrato mensal BTG BR PDF

Helpers internos:
  - _dec, _date_br, _pdf_lines_by_y, _pdf_text, _pdf_tables
  - _save_to_history (state in-memory; substituir por DB no futuro)
"""
...
# State in-memory (precisa ficar aqui pq _save_to_history foi extraido):
# import_route.py ainda referencia via re-import.
