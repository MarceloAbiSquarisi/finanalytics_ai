...
async def get_status() -> dict[str, Any]:
    try:
        ...
        inst = next(
            (i for i in instances if i.get("instance", {}).get("instanceName") == INSTANCE_NAME),
            None,
        )
        if inst:
            return {
                "connected": inst.get("instance", {}).get("state") == "open",
                "state": inst.get("instance", {}).get("state", "unknown"),
                "number": inst.get("instance", {}).get("owner", ""),
...
async def send_message(body: SendRequest) -> dict[str, Any]:
    ...
    try:
        data = await _evo_post(
            f"/message/sendText/{INSTANCE_NAME}", {"number": number, "text": body.message}
        )
...
async def criar_alerta_preco(body: AlertaPreco) -> dict[str, Any]:
    ...
    alerta = {
        ...
        "number": body.number,
        "mensagem_extra": body.mensagem_extra,
        "ativo": True,
    }
...
async def criar_alerta_setup(body: AlertaSetup) -> dict[str, Any]:
    ...
    alerta = {
        ...
        "setups": body.setups,
        "number": body.number,
        "intervalo_min": body.intervalo_min,
        "ativo": True,
    }
    _alertas.append(alerta)
    return {"success": True, "alerta_id": _alerta_id, "alerta": alerta}
