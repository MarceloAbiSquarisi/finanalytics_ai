...
async def buscar_fundos(
    ...
    total_r = await session.execute(
        text(f"SELECT COUNT(*) FROM fundos_cadastro WHERE {where}"), params
    )
        ...
        "tipos": [
            {
                "tipo": r.tipo,
                "total": r.total,
                "pl_total": float(r.pl_total) if r.pl_total else None,
            }
            for r in rows
        ]
    }
...
async def historico_cotas(
    ...
    candles = [
        {
            "time": int(
                datetime.combine(r.data_ref, datetime.min.time()).replace(tzinfo=UTC).timestamp()
            ),
            "value": float(r.vl_quota),
        }
        for r in rows
    ]
...
async def detalhe_fundo(
    cnpj: str, session: AsyncSession = Depends(get_db_session)
) -> dict[str, Any]:
    row = await session.execute(
        text("SELECT * FROM fundos_cadastro WHERE cnpj = :cnpj"), {"cnpj": cnpj.strip()}
    )
    ...
    if rent:
        for k, v in dict(rent).items():
            if hasattr(v, "isoformat"):
                rentabilidade[k] = v.isoformat()
            elif hasattr(v, "__float__") and v is not None:
                rentabilidade[k] = float(v)
            else:
                rentabilidade[k] = v
    ...
    historico = [
        {
            ...
            "cota": float(r.vl_quota) if r.vl_quota else None,
