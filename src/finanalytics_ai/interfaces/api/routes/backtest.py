...
def _build_strategy_params(req: BacktestRequest) -> dict[str, Any]:
    ...
    if req.rsi_period     is not None: mapping["period"]        = req.rsi_period
    if req.rsi_oversold   is not None: mapping["oversold"]      = req.rsi_oversold
    if req.rsi_overbought is not None: mapping["overbought"]    = req.rsi_overbought
    if req.macd_fast      is not None: mapping["fast"]          = req.macd_fast
    if req.macd_slow      is not None: mapping["slow"]          = req.macd_slow
    if req.macd_signal    is not None: mapping["signal_period"] = req.macd_signal

    # Combined usa prefixo rsi_/macd_
    if req.strategy == "combined":
        combined: dict[str, Any] = {}
        if req.rsi_period     is not None: combined["rsi_period"]     = req.rsi_period
        if req.rsi_oversold   is not None: combined["rsi_oversold"]   = req.rsi_oversold
        if req.rsi_overbought is not None: combined["rsi_overbought"] = req.rsi_overbought
        if req.macd_fast      is not None: combined["macd_fast"]      = req.macd_fast
        if req.macd_slow      is not None: combined["macd_slow"]      = req.macd_slow
        if req.macd_signal    is not None: combined["macd_signal"]    = req.macd_signal
        return combined
...
async def run_backtest_get(
    ...
    body = BacktestRequest(
        ...
        commission_pct=commission_pct, rsi_period=rsi_period,
...
async def compare_multi_ticker_get(
    ...
    body = MultiTickerRequest(
        tickers=ticker_list, strategy=strategy, range_period=range_period,
        initial_capital=initial_capital, position_size=position_size,
        commission_pct=commission_pct, objective=objective, top_n=top_n,
    )
    svc = _get_multi_ticker(request)
