...
# Pool persistente: evita TCP handshake + auth a cada request.
# Lazy init no primeiro uso; reutilizado por todos os endpoints.
# Com _doRefresh a 500ms isso representa 2 req/s — sem pool cada uma
# pagaria ~15-25ms so de conexao; com pool cai para ~2-5ms por query.
...
async def get_last_candle(
    ...
    Retorna timestamptz UTC (sem AT TIME ZONE nem to_char) para que o
    frontend converta corretamente via new Date(ts).getTime()/1000.
    """
    bucket = _INTERVALS[resolution]
    try:
        async with (await _get_pool()).acquire() as conn:
