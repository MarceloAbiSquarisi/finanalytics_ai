...
async def yield_curve(
    ...
    """
    return await get_yield_curve_with_stress(selic=selic / 100, cdi=cdi / 100, ipca=ipca / 100)
    ...
    async with SM() as s, s.begin():
        ...
        for nome, dados in all_data.items():
            ...
            for row in dados:
                ...
                if dt is None or val is None or not isinstance(dt, _date):
                    continue
                await s.execute(
                    _text("""
            ...
            if dados:
                ...
                if ultimo:
                    anu = (
                        round(((1 + ultimo / 100) ** 252 - 1) * 100, 4)
                        if "diaria" in nome or "diario" in nome
                        else round(((1 + ultimo / 100) ** 12 - 1) * 100, 4)
                    )
...
async def get_historico_taxa(
    indicador: str = Query(
        "selic_diaria", description="selic_diaria|cdi_diario|ipca_mensal|igpm_mensal"
    ),
...
async def list_rf_portfolios(
    user_id: str = Query(..., min_length=1), session: AsyncSession = Depends(get_db_session)
) -> list[dict]:
...
async def delete_rf_portfolio(
    portfolio_id: str, session: AsyncSession = Depends(get_db_session)
) -> None:
...
async def add_rf_holding(
    portfolio_id: str, body: AddHoldingRequest, session: AsyncSession = Depends(get_db_session)
) -> dict:
...
async def delete_rf_holding(
    portfolio_id: str, holding_id: str, session: AsyncSession = Depends(get_db_session)
) -> None:
