...
def evalDllReturn(function: str, ret: int) -> bool:
    if ret < NL_OK:
        print(f"{function}: {NResultToString(ret)}")
...
def printOrder(title: str, orderId: TConnectorOrderIdentifier):
    order = TConnectorOrderOut(Version=0, OrderID=orderId)
    ...
    print(
        f"{title}: {order.AssetID.Ticker} | {order.TradedQuantity} | {order.OrderSide} | {order.Price} | {order.AccountID.AccountID} | {order.AccountID.SubAccountID} | {order.OrderID.ClOrderID} | {order.OrderStatus} | {order.TextMessage}"
    )
...
def priceDepthCallback(
    assetId: TConnectorAssetIdentifier, side: int, position: int, updateType: int
):
    if side == 0:
        sideName = "Buy"
    elif side == 1:
    ...
    match updateType:
        ...
        case 1:  # Edit
            ...
            if profit_dll.GetPriceGroup(byref(assetId), side, position, byref(price)) == NL_OK:
                if price.PriceGroupFlags & 1:  # theoric
                    ...
                    theoricQty = c_int64()
                    if (
                        profit_dll.GetTheoreticalValues(
                            byref(assetId), byref(theoricPrice), byref(theoricQty)
                        )
                        == NL_OK
                    ):
                ...
                print(
                    f"PriceDepthCallback: {assetId.Ticker} | {sideName} : Edit | {price.Price} : {price.Count} : {price.Quantity}"
                )
            pass
        case 2:  # Delete
            print(
                f"PriceDepthCallback: {assetId.Ticker} | {sideName} : Delete | Position={position}"
            )
            pass
        case 3:  # Insert
            price = TConnectorPriceGroup()
        ...
        case 4:  # FullBook
            ...
            if count >= 0:
                print(
                    f"PriceDepthCallback: {assetId.Ticker} | Buy : FullBook | Count={count}"
                )

            count = profit_dll.GetPriceDepthSideCount(byref(assetId), 1)
            if count >= 0:
                print(
                    f"PriceDepthCallback: {assetId.Ticker} | Sell : FullBook | Count={count}"
                )
            pass
        case 5:  # Prepare
            pass
        ...
        case 8:  # DeleteFrom
            print(
                f"PriceDepthCallback: {assetId.Ticker} | {sideName} : DeleteFrom | Position={position}"
            )
            pass
    return
...
def tradingMessageResultCallback(a_Result):
    ...
    print(
        f"TradingMessageResultCallback: {result.BrokerID} | {result.MessageID} | {result.Message}"
    )
    return
...
def descript_offer_array_v2(offer_array):
    ...
    for i in range(qtd_offer):
        price, qtd, agent, offer_id, date_length = struct.unpack(
            "=dqiqH", frame[start : start + 30]
        )
        start += 30

        date_str = struct.unpack(f"{date_length}s", frame[start : start + date_length])[0].decode(
            "ansi"
        )
        start += date_length
...
def adjustHistoryCallbackV2(
    assetId, value, strType, strObserv, dtAjuste, dtDelib, dtPagamento, nFlags, dMult
):
...
def assetListInfoCallbackV2(
    ...
    print(
        "TAssetListInfoCallbackV2 = Ticker: "
        + str(assetId.ticker)
        + "Name: "
        + str(strName)
        + "Descrição: "
...
def PrintEnumOrders(order_ptr, param):
    ...
    print(
        f"OrderHistoryCallback: {order.AssetID.Ticker} | {order.TradedQuantity} | {order.OrderSide} | {order.Price} | {order.AccountID.AccountID} | {order.AccountID.SubAccountID} | {order.OrderID.ClOrderID} | {order.OrderStatus} | {order.TextMessage}"
    )
...
def orderHistoryCallback(accountId: TConnectorAccountIdentifier):
    profit_dll.EnumerateAllOrders(
        byref(accountId), 0, 0, TConnectorEnumerateOrdersProc(PrintEnumOrders)
    )
...
def PrintEnumAssets(asset_ptr, param):
    ...
    print(
        f"OrderHistoryCallback: {asset.Ticker} | {asset.Exchange} | {asset.FeedType}"
    )
...
def getAssetsPositionCallback(
    accountId: TConnectorAccountIdentifier, asset: TConnectorAssetIdentifier, LastEvent: c_long
):

    print(f"Account: {accountId.BrokerID} | {accountId.AccountID}")

    print(f"Asset: {asset.Ticker} | {asset.Exchange} | {asset.FeedType}")
...
def BrokerAccountListChangedCallback(BrokerID, HasChange):
    ...
    if IsErrorCode(count_return):
        print(
            f"BrokerAccountListChangedCallback {BrokerID} | Error={NResultToString(count_return)}"
        )
    else:
        print(f"BrokerAccountListChangedCallback {BrokerID} | Count={count_return}")
...
def BrokerSubAccountListChangedCallback(accountId: TConnectorAccountIdentifier):
    ...
    if IsErrorCode(count_return):
        print(
            f"BrokerSubAccountListChangedCallback {accountId.BrokerID} | Account: {accountId.AccountID} | Error={NResultToString(count_return)}"
        )
    else:
        print(
            f"BrokerSubAccountListChangedCallback {accountId.BrokerID} | Account: {accountId.AccountID} | Count={count_return}"
        )
...
def SenSellOrder():
    ...
    nProfitID = profit_dll.SendSellOrder(
        c_wchar_p("CONTA"),
        c_wchar_p("BROKER"),
        c_wchar_p("PASS"),
        c_wchar_p("ATIVO"),
...
def subscribePriceDepth():
    ...
    assetID = TConnectorAssetIdentifier(Version=0, Ticker=asset, Exchange=exchange, FeedType=0)
...
def doGetTheoricPrice():
    ...
    print(
        "GetTheoreticalValues: Result="
        + str(result)
        + " Price="
        + str(theoricPrice.value)
        + " Qty="
        + str(theoricQty.value)
    )
...
def printLastAdjusted():
    ...
    print(f"Last session close: {close}, result={result!s}")
...
def printPosition():
    ...
    position = TConnectorTradingAccountPosition(Version=1, PositionType=positionType)
    position.AccountID = TConnectorAccountIdentifier(
        Version=0, BrokerID=brokerId, AccountID=accountId, SubAccountID=subAccountId
    )
    ...
    print(
        f"Price: {position.OpenAveragePrice} | AvgSellPrice: {position.OpenAveragePrice} | AvgBuyPrice: {position.OpenAveragePrice} | SellQtd: {position.OpenAveragePrice} | BuyQtd: {position.OpenAveragePrice}"
    )
...
def doZeroPosition():
    ...
    zeroRec = TConnectorZeroPosition(
        Version=2, PositionType=positionType, Password=rotPassword, Price=-1.0, MessageID=-1
    )
    ...
    print(f"ZeroOrderID: {ret} | MessageID=")
...
def buyStopOrder():
    ...
    send_order.AccountID = TConnectorAccountIdentifier(
        Version=0, BrokerID=brokerId, AccountID=accountId, SubAccountID=subAccountId, Reserved=0
    )
...
def getOrder():
    ...
    order_id = TConnectorOrderIdentifier(Version=0, LocalOrderID=profit_id, ClOrderID=cl_ord_id)
...
def cancelOrder():
    ...
    cancel_order = TConnectorCancelOrder(Version=1, Password=rotPassword, MessageID=-1)
    cancel_order.OrderID = TConnectorOrderIdentifier(
        Version=0, LocalOrderID=-1, ClOrderID=cl_ord_id
    )
...
def cancelAllOrders():
    ...
    cancel_order = TConnectorCancelAllOrders(Version=0, Password=rotPassword)
    cancel_order.AccountID = TConnectorAccountIdentifier(
        Version=0, BrokerID=brokerId, AccountID=accountId, SubAccountID=subAccountId, Reserved=0
    )
...
def changeOrder():
    ...
    change_order = TConnectorChangeOrder(
        Version=1, Password=rotPassword, Price=price, StopPrice=-1, Quantity=amount, MessageID=-1
    )
...
def getAccount():
    ...
    for i in range(count):
        accountID = TConnectorAccountIdentifier(
            Version=0, BrokerID=accountIDs[i].BrokerID, AccountID=accountIDs[i].AccountID
        )
        ...
        if account:
            print(
                f"GetAccount: {accountID.BrokerID} | {accountID.AccountID} | {account.OwnerName} | Type={account.AccountType}"
            )
            ...
            if subCount > 0:
                ...
                for j in range(subCount):
                    ...
                    if subAccount:
                        print(
                            f"GetAccount-Sub: {subAccountID.BrokerID} | {subAccountID.AccountID}-{subAccountID.SubAccountID} | {subAccount.SubOwnerName} | Type={subAccount.AccountType}"
                        )
...
def doHasOrdersInInterval():
    ...
    accountId = TConnectorAccountIdentifier(
        Version=0, BrokerID=brokerId, AccountID=accountId, SubAccountID="", Reserved=0
    )

    today = datetime_to_systemtime(
        datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    )
    yesterday = datetime_to_systemtime(
...
def GetPositionAssets():
    ...
    if subAccountID != "":
        ...
        accountObject = TConnectorAccountIdentifier(
            Version=0, BrokerID=brokerID, AccountID=accountID
        )

    profit_dll.EnumerateAllPositionAssets(
        byref(accountObject), 0, 0, TConnectorEnumerateAssetProc(PrintEnumAssets)
    )
