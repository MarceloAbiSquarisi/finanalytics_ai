...
async def get_taxas_atuais(session: AsyncSession) -> dict[str, float]:
    ...
    try:
        ...
        res = dict(defaults)
        if "selic_diaria" in data:
            res["selic"] = round(((1 + data["selic_diaria"] / 100) ** 252 - 1) * 100, 2)
        if "cdi_diario" in data:
            res["cdi"] = round(((1 + data["cdi_diario"] / 100) ** 252 - 1) * 100, 2)
        if "ipca_mensal" in data:
            res["ipca"] = round(((1 + data["ipca_mensal"] / 100) ** 12 - 1) * 100, 2)
        if "igpm_mensal" in data:
            res["igpm"] = round(((1 + data["igpm_mensal"] / 100) ** 12 - 1) * 100, 2)
        return res
    except:
        return defaults
