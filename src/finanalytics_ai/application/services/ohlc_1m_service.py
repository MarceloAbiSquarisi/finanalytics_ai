...
from finanalytics_ai.infrastructure.database.repositories.ohlc_1m_repo import OHLC1mMetaModel, OHLC1mModel
...
class OHLC1mService:
    ...
    async def get_bars(self, ticker: str, interval: str = "5m", range_period: str = "5d") -> list[dict[str, Any]]:
        ticker = ticker.upper()
    ...
    async def _refresh(self, s: AsyncSession, ticker: str) -> int:
        try:
            ...
            logger.warning("ohlc_1m.fail", ticker=ticker, error=str(e)); return 0
        if not bars: return 0
        rows = [{"ticker": ticker, "timestamp": int(b["time"]), "open": b["open"],
                 "high": b["high"], "low": b["low"], "close": b["close"], "volume": b.get("volume") or 0}
                for b in bars]
        st = pg_insert(OHLC1mModel).values(rows)
        await s.execute(st.on_conflict_do_update(index_elements=["ticker","timestamp"],
            set_={"open": st.excluded.open, "high": st.excluded.high,
                  "low": st.excluded.low, "close": st.excluded.close, "volume": st.excluded.volume}))
        now = int(time.time())
        ms = pg_insert(OHLC1mMetaModel).values(ticker=ticker, last_fetch_at=now,
            oldest_bar_ts=min(r["timestamp"] for r in rows),
            newest_bar_ts=max(r["timestamp"] for r in rows), bar_count=len(rows))
        await s.execute(ms.on_conflict_do_update(index_elements=["ticker"],
            set_={"last_fetch_at": now, "newest_bar_ts": max(r["timestamp"] for r in rows), "bar_count": len(rows)}))
        await s.commit()
        logger.info("ohlc_1m.refreshed", ticker=ticker, bars=len(rows))
        return len(rows)

    async def _load(self, s, ticker, range_period):
        _M = {"1d": 86400, "5d": 432000, "1mo": 2592000, "3mo": 7776000, "6mo": 15552000, "1y": 31536000, "2y": 63072000}
        d = _M.get(range_period)
        q = select(OHLC1mModel).where(OHLC1mModel.ticker == ticker).order_by(OHLC1mModel.timestamp)
        if d: q = q.where(OHLC1mModel.timestamp >= int(time.time()) - d)
        r = await s.execute(q)
        return [{"time": x.timestamp, "open": x.open, "high": x.high, "low": x.low, "close": x.close, "volume": x.volume or 0} for x in r.scalars()]

    async def _daily(self, ticker, interval, range_period):
        try:
            return await self._brapi.get_ohlc_bars(Ticker(ticker), range_period=range_period, interval=interval)
        except Exception as e:
            logger.warning("ohlc_daily.fail", ticker=ticker, error=str(e)); return []
