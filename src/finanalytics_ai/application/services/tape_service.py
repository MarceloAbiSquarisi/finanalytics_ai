...
class TapeService:
    ...
    def _empty_metrics(self, ticker: str) -> TapeMetrics:
        return TapeMetrics(
            ticker=ticker, last_price=0, total_trades=0, total_volume=0,
            vol_compra=0, vol_venda=0, ratio_cv=1.0, saldo_fluxo=0,
            pct_agressao_compra=50, trades_por_min=0,
            preco_max=0, preco_min=0, nivel_dominante="EQUILIBRIO",
            updated_at=datetime.now(timezone.utc).isoformat(),
        )
        ...
        while time.time() < end_time:
            for ticker in tickers:
                ...
                tick = TradeTick(
                    ...
                    volume=float(vol), quantity=vol // 100,
