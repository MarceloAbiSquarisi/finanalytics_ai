...
class RFPortfolioService:
    ...
    async def create_portfolio(self, user_id: str, name: str) -> dict[str, Any]:
        ...
        return {"portfolio_id": p.portfolio_id, "user_id": p.user_id,
    ...
    async def add_holding(
        ...
        holding = await self._repo.add_holding(
            ...
            ir_exempt        = ir_exempt,       # type: ignore
            note             = note,
        )
    ...
    async def diversification_report(self, portfolio_id: str) -> Optional[dict[str, Any]]:
        p = await self._repo.get_portfolio(portfolio_id)
        if p is None:
            return None
        ...
        return {
            ...
            "alerts": [
                {"alert_type": a.alert_type, "name": a.name, "pct": a.pct,
    ...
    async def maturities_timeline(
        ...
        if p is None:
            return None
        result = []
        for h in sorted(p.active_holdings,
                         key=lambda x: x.maturity_date or date(2099, 12, 31)):
            idx_rate = {
    ...
    def _portfolio_detail(
        self, p: RFPortfolio,
        selic: float, cdi: float, ipca: float,
    ) -> dict[str, Any]:
