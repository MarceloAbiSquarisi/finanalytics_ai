...
from finanalytics_ai.domain.patrimony.consolidated import (
    AssetClass, build_snapshot, ConsolidatedSnapshot, DEFAULT_TARGETS,
)
...
_ETF_TICKERS = {
    "BOVA11","SMAL11","DIVO11","FIND11","MATB11","UTIP11","ECOO11","BOVV11","XBOV11",
    "IVVB11","SPXI11","NASD11","ACWI11","EURP11",
    "HASH11","QBTC11","BITH11",
    "NTNB11","IMAB11","B5P211","IRFM11",
    "GOLD11","OGLD11",
    "XFIX11","VISC11",
}
...
class PatrimonyService:
    ...
    async def consolidated_snapshot(
        ...
        """
        from finanalytics_ai.domain.fixed_income.entities import BondType, Indexer, PaymentFrequency, Bond
        ...
        try:
                ...
                async def _price(t: str) -> tuple[str, float]:
                    async with sem:
                        try:
                            ...
                            if bars:
                                return t, float(bars[-1].get("close") or bars[-1].get("regularMarketPrice") or 0)
                        except Exception:
                            pass
                        return t, 0.0
            ...
            for p in portfolios:
                for pos in p.positions:
                    ...
                    pl    = val - inv
                    detail = {
                        "ticker": t, "quantity": float(pos.quantity.value),
                        "avg_price": float(pos.average_price.amount),
                        "current_price": price, "current_value": round(val, 2),
                        "invested": round(inv, 2), "pl": round(pl, 2),
                    ...
                    if t in _ETF_TICKERS:
                        etf_value    += val; etf_invested += inv; etf_positions += 1
                    else:
                        eq_value  += val; eq_invested  += inv; eq_positions  += 1
            ...
            for pf_summary in rf_portfolios:
                ...
                for h in pf.active_holdings:
                    ...
                    try:
                        ...
                        idx = Indexer.CDI
                    bond = Bond(
                        bond_id=h.bond_id, name=h.bond_name, bond_type=bt,
                        indexer=idx, rate_annual=h.rate_annual,
                        rate_pct_indexer=h.rate_pct_indexer,
                        maturity_date=h.maturity_date, issuer=h.issuer,
                        ir_exempt=h.ir_exempt,
                        payment_freq=PaymentFrequency.AT_MATURITY,
                    )
                    idx_rate = {"CDI": cdi, "SELIC": selic, "IPCA": ipca}.get(h.indexer, cdi)
                    yr = calculate_yield(
                        bond=bond, principal=h.invested,
                        days=max(1, h.days_held),
                        indexer_rate=idx_rate, inflation_rate=ipca,
                    )
                    ...
                    rf_details.append({
                        ...
                        "bond_type": h.bond_type, "issuer": h.issuer,
        ...
        snap = build_snapshot(
            user_id=user_id,
            equities_value=eq_value,    equities_invested=eq_invested,  equities_positions=eq_positions,
            etfs_value=etf_value,       etfs_invested=etf_invested,     etfs_positions=etf_positions,
            rf_value=rf_value,          rf_invested=rf_invested,        rf_positions=rf_positions,
        ...
        log.info("patrimony.done",
                 total=round(snap.total_value, 2),
                 pl_pct=round(snap.total_pl_pct, 2))
        return result
            ...
            for pf_summary in rf_portfolios:
                ...
                for h in pf.active_holdings:
                    ...
                    advice = analyze_ir_timing(
                        holding_id=h.holding_id, bond_name=h.bond_name,
                        bond_type=h.bond_type, issuer=h.issuer,
                        invested=h.invested, purchase_date=h.purchase_date,
                        maturity_date=h.maturity_date,
                        rate_annual=h.rate_annual,
                        rate_pct_indexer=h.rate_pct_indexer,
                        indexer=h.indexer, indexer_rate=idx_rate,
                        inflation_rate=ipca,
                    )
                    results.append({
