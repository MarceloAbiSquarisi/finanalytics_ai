...
ETF_CATALOG: list[ETFInfo] = [
    ...
    ETFInfo("DIVO11", "iShares Dividendos",         "IDIV",   "Ações BR",    0.20, "BRL", "Ações com maiores dividendos"),
    ETFInfo("FIND11", "iShares Financeiro",         "IFNC",   "Ações BR",    0.50, "BRL", "Setor financeiro B3"),
    ETFInfo("MATB11", "iShares Materiais Básicos",  "IMAT",   "Ações BR",    0.50, "BRL", "Materiais básicos B3"),
    ETFInfo("UTIP11", "iShares Utilidade Pública",  "UTIL",   "Ações BR",    0.50, "BRL", "Utilidades B3"),
    ETFInfo("ECOO11", "iShares Carbono Eficiente",  "ICO2",   "Ações BR",    0.40, "BRL", "Empresas de baixo carbono"),
    ...
    # ── Cripto ────────────────────────────────────────────────────────────────
    ETFInfo("HASH11", "Hashdex Cripto",             "BTC-USD","Cripto",       0.95, "BRL", "Cesta cripto (BTC+ETH+outros)"),
...
class ETFMetrics:
    ...
    def sharpe_label(self) -> str:
        if self.sharpe >= 2.0: return "Excelente"
        if self.sharpe >= 1.0: return "Bom"
        if self.sharpe >= 0.5: return "Razoável"
        if self.sharpe >= 0.0: return "Fraco"
        return "Negativo"
...
class TrackingErrorResult:
    ...
    def quality_label(self) -> str:
        if self.tracking_error_pct < 0.5:  return "Excelente replicação"
        if self.tracking_error_pct < 1.5:  return "Boa replicação"
        if self.tracking_error_pct < 3.0:  return "Replicação razoável"
