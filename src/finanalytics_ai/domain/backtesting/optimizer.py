...
def grid_search(
    ...
    if not space:
        raise ValueError(
            f"Sem espaco de parametros para '{strategy_name}'. Disponiveis: {list(PARAM_SPACES)}"
        )
...
def _filter_invalid(
    ...
    filtered = []
    for combo in combos:
        p = dict(zip(names, combo, strict=False))
        if strategy == "ema_cross" and p.get("fast", 0) >= p.get("slow", 1):
            continue
        if strategy == "macd" and p.get("fast", 0) >= p.get("slow", 1):
            continue
...
def _build_heatmap(
    ...
    for params, metrics, _run_score, is_valid in raw_results:
        if not is_valid:
            continue
    ...
    x_vals = sorted({k[0] for k in cell_scores})
    y_vals = sorted({k[1] for k in cell_scores})
...
def walk_forward(
    ...
    if oos_size < 10:
        raise ValueError(
            f"Janela OOS muito pequena ({oos_size} barras). Aumente o periodo ou reduza n_splits."
        )
    ...
    for split_idx in range(n_splits):
        ...
        # Expanding (anchored): IS começa em 0; Rolling: tamanho fixo
        is_start = 0 if anchored else max(0, oos_start - is_size)
    ...
    # ── Metricas de robustez ──────────────────────────────────────────────────
    [f for f in folds if f.oos_valid]
