    ...
    def evaluate(self, current_price: Decimal) -> AlertTriggerResult:
        ...
        if self.status != AlertStatus.ACTIVE:
            return AlertTriggerResult(
                triggered=False, alert_id=self.alert_id,
                ticker=self.ticker, alert_type=self.alert_type,
                message="Alerta inativo", current_price=current_price,
        ...
        if self.expires_at and datetime.utcnow() > self.expires_at:
            return AlertTriggerResult(
                triggered=False, alert_id=self.alert_id,
                ticker=self.ticker, alert_type=self.alert_type,
                message="Alerta expirado", current_price=current_price,
                threshold=self.threshold,
            )
        ...
        match self.alert_type:
            case AlertType.STOP_LOSS:
                if price <= t:
                    ...
                    return True, (
                        f"🔴 STOP LOSS {self.ticker}: R$ {price:.2f} ≤ R$ {t:.2f} "
                        f"(queda de {loss:.1f}%)"
                    ), {"loss_pct": str(loss)}
            ...
            case AlertType.TAKE_PROFIT:
                if price >= t:
                    ...
                    return True, (
                        f"🟢 TAKE PROFIT {self.ticker}: R$ {price:.2f} ≥ R$ {t:.2f} "
                        f"(ganho de {gain:.1f}%)"
                    ), {"gain_pct": str(gain)}
            ...
            case AlertType.PRICE_TARGET:
                ...
                if pct <= Decimal("0.1"):   # dentro de 0.1% do alvo
                    return True, (
                        f"🎯 ALVO {self.ticker}: R$ {price:.2f} ≈ R$ {t:.2f}"
                    ), {}
