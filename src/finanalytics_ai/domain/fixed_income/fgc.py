...
def analyze_fgc(portfolio_id: str, holdings: list[RFHolding]) -> FGCAnalysis:
    ...
    for issuer, data in inst_map.items():
        ...
        if excess == 0.0:
            ...
            level = "warning"
            msg = (
                f"{issuer}: R$ {total:,.0f} investidos — "
        ...
        institutions.append(
            FGCInstitutionStatus(
                issuer=issuer,
                total_invested=total,
                fgc_covered=round(covered, 2),
                fgc_uncovered=round(excess, 2),
                within_limit=within,
    ...
    for h in active:
        ...
        if cov == "fgc":
            ...
            msg = (
                ...
                if within
                else f"Parte sem cobertura FGC: ~R$ {holding_excess:,.0f} desprotegidos "
            ...
            level = "info"
            msg = (
                "Tesouro Direto: garantia soberana do Governo Federal. "
            ...
            level = "critical"
            msg = (
                f"{h.bond_type}: não coberto pelo FGC. "
        ...
        holding_statuses.append(
            FGCHoldingStatus(
                ...
                excess_amount=round(holding_excess, 2),
                alert_level=level,
                alert_message=msg,
            )
        )
    ...
    if uncovered_holdings:
        ...
        alerts.append(
            {
                "type": "no_fgc_coverage",
                "level": "critical",
                "message": (
                    f"{len(uncovered_holdings)} título(s) SEM cobertura FGC: {names}. "
        ...
        alerts.append(
            {
                ...
                "level": "warning",
                "message": (
                    f"Total coberto pelo FGC (R$ {total_fgc:,.0f}) supera o "
