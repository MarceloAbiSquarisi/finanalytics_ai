...
def analyze_ir_timing(
    ...
    if exempt:
        rec = (f"{bond_name} é isento de IR. "
        ...
        rec = (f"A economia fiscal potencial é de R$ {max_saving:.2f} — pequena para este montante. "
               "O timing tributário tem pouco impacto neste título.")
    else:
        days_wait = (best.redemption_date - today).days
        rec = (f"Aguardar até {best.redemption_date.strftime('%d/%m/%Y')} "
        ...
        if best.label == "Após 30 dias (fim IOF)":
            rec = (f"⚠️ IOF ativo! Aguardar até {best.redemption_date.strftime('%d/%m/%Y')} "
