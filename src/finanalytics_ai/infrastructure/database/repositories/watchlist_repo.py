...
import structlog
from sqlalchemy import Column, DateTime, ForeignKey, String, Text, Float, select, delete
...
class WatchlistAlertModel(Base):
    ...
    item_id           = Column(String(36),  ForeignKey("watchlist_items.item_id", ondelete="CASCADE"),
...
class WatchlistRepository:
    ...
    async def save_item(self, item: WatchlistItem) -> None:
        ...
        if existing:
            ...
            self._session.add(WatchlistItemModel(
    ...
    async def save_alert(self, alert: SmartAlert, item_id: str) -> None:
        ...
        if existing:
            ...
            self._session.add(WatchlistAlertModel(
    ...
    async def find_all_active_alerts(self) -> list[tuple[SmartAlert, str]]:
        ...
        stmt   = select(WatchlistAlertModel).where(
            WatchlistAlertModel.status.in_([
                SmartAlertStatus.ACTIVE, SmartAlertStatus.COOLDOWN
            ])
        )
        result = await self._session.execute(stmt)
