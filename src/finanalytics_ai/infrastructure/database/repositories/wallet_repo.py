...
class WalletRepository:
    ...
    async def get_positions_summary(self, user_id: str,
        ...
        trades = await self.list_trades(user_id, asset_class=asset_class)
    ...
    async def list_crypto(self, user_id: str) -> list[dict]:
        async with get_session() as s:
            q = select(CryptoHoldingModel).where(
                CryptoHoldingModel.user_id == user_id).order_by(CryptoHoldingModel.symbol)
            rows = (await s.execute(q)).scalars().all()
            return [_model_to_dict(r) for r in rows]
