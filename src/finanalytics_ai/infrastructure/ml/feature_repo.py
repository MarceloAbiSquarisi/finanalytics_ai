...
class SqlFeatureRepository:
    ...
    async def get_fundamental_features(
        ...
        result: dict[str, float | None] = dict.fromkeys(indicadores)
        for row in rows.mappings():
            v = row["valor"]
            result[row["indicador"]] = float(v) if v is not None else None
        return result
        ...
        for m in metrics_list:
            ...
            await self._session.execute(
                sql,
                {
                    ...
                    "var_h": m.var_95_historical,
                    "cvar_h": m.cvar_95_historical,
                    "var_p": m.var_95_parametric,
                    "cvar_p": m.cvar_95_parametric,
                    "t_df": m.t_degrees_of_freedom,
                    "var_g": m.var_95_garch,
                    "cvar_g": m.cvar_95_garch,
                    "garch_vol": m.garch_volatility_forecast,
                    "var_mc": m.var_95_mc,
                    "cvar_mc": m.cvar_95_mc,
