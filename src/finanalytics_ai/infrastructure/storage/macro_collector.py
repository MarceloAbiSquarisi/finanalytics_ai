"""
MacroCollector — Coleta dados macroeconômicos do BCB, Yahoo Finance e Focus.
...
Séries coletadas:
  ...
  Boletim Focus (BCB Olinda API — gratuito, sem autenticação):
    - Expectativas: IPCA, Selic, PIB Total, Câmbio, IGP-M
    - Snapshot resumido para o dashboard macro
...
class MacroCollector:
    ...
    async def collect_all(self) -> dict[str, Any]:
        ...
        # ── Boletim Focus ────────────────────────────────────────────────────
        try:
            rows = await self._collect_focus()
            report["focus"] = {"source": "BCB/Focus", "rows": rows, "status": "ok"}
            logger.info("macro.focus.ok", rows=rows)
        except Exception as e:
            report["focus"] = {"source": "BCB/Focus", "status": "error", "error": str(e)[:80]}
            logger.warning("macro.focus.error", error=str(e)[:80])

        return report

    async def _collect_focus(self) -> int:
        """Coleta snapshot do Boletim Focus e persiste como parquet."""
        from finanalytics_ai.infrastructure.adapters.focus_client import get_focus_client

        focus = get_focus_client()
        expectations = await focus.get_latest_expectations(semanas=12)
        ...
        # Salva série geral de Focus
        df_simple = df[["date", "value"]].copy()
        self._storage.write_macro("focus", df_simple)
        ...
        for indicador, col_key in _indicator_keys.items():
            ...
            if not df_ind.empty:
                self._storage.write_macro(col_key, df_ind)

        logger.info("macro.focus.persisted", rows=len(df), series=len(_indicator_keys))
        return len(df)
