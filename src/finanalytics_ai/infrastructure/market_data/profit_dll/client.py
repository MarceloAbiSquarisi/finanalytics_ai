...
class ProfitDLLClient:
    ...
    async def start(self, loop: asyncio.AbstractEventLoop | None = None) -> None:
        """Inicializa a DLL e aguarda conexão com o servidor de Market Data."""
    ...
    def _register_callbacks(self) -> None:
        ...
        def state_callback(conn_type: int, result: int) -> None:
            self._on_state(conn_type, result)
    ...
    def _initialize(self) -> None:
        """Chama DLLInitializeMarketLogin com todos os callbacks obrigatórios."""
        assert self._dll is not None
    ...
    # ── Callbacks (chamados na ConnectorThread) ────────────────────────────────
