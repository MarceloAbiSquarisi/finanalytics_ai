...
class ImportAccount(Base):
    ...
    positions = relationship(
        "ImportPosition", back_populates="account", cascade="all, delete-orphan"
    )
    transactions = relationship(
        "ImportTransaction", back_populates="account", cascade="all, delete-orphan"
    )
    notes = relationship("ImportNote", back_populates="account", cascade="all, delete-orphan")
...
class ImportPosition(Base):
    ...
    id = Column(UUID(as_uuid=False), primary_key=True, default=_uuid)
    account_id = Column(
        UUID(as_uuid=False), ForeignKey("import_accounts.id", ondelete="CASCADE"), nullable=False
    )
    ticker = Column(String(100), nullable=False)
    nome = Column(String(300), nullable=True)
    tipo = Column(
        String(30), nullable=False
    )  # acao, fii, fundo, cdb, cra, cri, deb, ntnb, cripto, etf, bdr, fip, lci, lca, coe, alt, emprestimo, stock
    moeda = Column(String(10), default="BRL")
...
class ImportTransaction(Base):
    ...
    tipo = Column(
        String(30), nullable=False
    )  # compra, venda, dividendo, jcp, amortizacao, deposito, saque, taxa, emprestimo
    ticker = Column(String(100), nullable=False)
    nome = Column(String(300), nullable=True)
    ...
    moeda = Column(String(10), default="BRL")
    nota_id = Column(
        UUID(as_uuid=False), ForeignKey("import_notes.id", ondelete="SET NULL"), nullable=True
    )
...
class ImportNote(Base):
    ...
    items = relationship("ImportNoteItem", back_populates="note", cascade="all, delete-orphan")
    transactions = relationship("ImportTransaction", back_populates="note")

    __table_args__ = (
        UniqueConstraint(
            "account_id", "numero_nota", "data_pregao", name="uq_note_account_numero_data"
        ),
...
class ImportNoteItem(Base):
    ...
    id = Column(UUID(as_uuid=False), primary_key=True, default=_uuid)
    note_id = Column(
        UUID(as_uuid=False), ForeignKey("import_notes.id", ondelete="CASCADE"), nullable=False
    )
