...
DATABASE_DSN = os.environ.get(
    "DATABASE_DSN", "postgresql://finanalytics:secret@finanalytics_postgres:5432/finanalytics"
)
...
TICKERS = [
    ...
    for t in os.environ.get(
        "INGESTOR_TICKERS", "PETR4,VALE3,ITUB4,BBDC4,WEGE3,BBAS3,ABEV3,MXRF11,HGLG11,BOVA11"
    ).split(",")
]
_shutdown = False
...
async def run() -> None:
    ...
    async def _one(t) -> None:
        async with sem:
            try:
                n = await svc.ingest(t)
                logger.debug("ok", t=t, n=n)
            except Exception as e:
                logger.warning("fail", t=t, e=str(e))
