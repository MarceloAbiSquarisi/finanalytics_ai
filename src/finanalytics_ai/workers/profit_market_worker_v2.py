...
async def run() -> None:
    ...
    if not os.path.exists(dll_path):
        log.error("profit_worker_v2.dll_not_found", path=dll_path)
        return
    ...
    try:
        ...
        log.info(
            "profit_worker_v2.connect_attempt_registered",
            count=_count + 1,
            limit=_MAX_DAILY_CONNECTS,
        )
        _rg.close()
    ...
    for i in range(60):
        if routing_connected:
            log.info("profit_worker_v2.routing_connected", attempts=i)
            break
    ...
    for i in range(60):
        if market_connected:
            log.info("profit_worker_v2.market_connected", attempts=i)
            break
        await asyncio.sleep(0.5)
    else:
