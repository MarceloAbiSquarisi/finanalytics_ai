...
def trade_cb(asset_id, p_trade, flags):
    global tick_count
    if not p_trade or bool(flags & 1):
        return
    trade = TConnectorTrade(Version=0)
    if dll.TranslateTrade(p_trade, byref(trade)) != NL_OK or trade.Price <= 0:
        return
    st = trade.TradeDate
    tick_count += 1
    tick = {
    ...
    if tick_count % 10 == 1:
        print(
            f"[TICK #{tick_count}] {tick['ticker']} "
            f"P={tick['price']:.4f} Q={tick['qty']} {tick['ts'][11:19]}",
            flush=True,
        )
