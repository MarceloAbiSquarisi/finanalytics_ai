"""diag_trade_raw_v2.py — usa import correto do projeto."""
import ctypes, os, sys, time
from ctypes import WINFUNCTYPE, c_int, c_size_t, c_uint, c_wchar_p
from dotenv import load_dotenv
load_dotenv(override=False)

# Adiciona src ao path para importar tipos do projeto
...
print(f"\n[RESULTADO] raw={len(raw_calls)} ok={len(translate_ok)} fail={len(translate_fail)}", flush=True)
dll.DLLFinalize()
