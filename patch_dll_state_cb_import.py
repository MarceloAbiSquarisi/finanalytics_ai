...
ANCHOR = '        @_WFTYPE(None, _cint, _cint)\n        def _state_cb(t, r):\n            import structlog as _sl\n            _cb_log = _sl.get_logger("profit_dll.state")\n'
REPLACEMENT = '        import structlog as _structlog_cb  # importado fora do callback — evita deadlock no GIL\n        _cb_log_ref = _structlog_cb.get_logger("profit_dll.state")\n\n        @_WFTYPE(None, _cint, _cint)\n        def _state_cb(t, r):\n            _cb_log = _cb_log_ref\n'
SENTINEL = '# importado fora do callback — evita deadlock no GIL'
