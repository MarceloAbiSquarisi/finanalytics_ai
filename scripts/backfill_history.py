...
def get_collected_dates(ticker: str, exchange: str, start: date, end: date) -> set[date]:
    """
        ...
        if last_to:
            try:
                ...
                if start <= last_date <= end:
                    ...
                    print(
                        f"[{ticker}] Resume a partir de {resume_from} "
                        f"(pulando {skipped} dias já coletados no range)"
                    )
                elif last_date > end:
                    print(
                        f"[{ticker}] last_collected_to={last_date} fora do range "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--start", default="2026-01-02", help="Data inicial YYYY-MM-DD (default: 2026-01-02)"
    )
    parser.add_argument(
        "--end", default="2026-03-31", help="Data final YYYY-MM-DD (default: 2026-03-31)"
    )
    parser.add_argument(
        "--delay",
        type=float,
    ...
    parser.add_argument("--dry-run", action="store_true", help="Simula sem fazer chamadas reais")
