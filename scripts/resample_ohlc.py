...
def parse_args() -> argparse.Namespace:
    ...
    g.add_argument("--all-tickers", action="store_true", help="Todos tickers em ohlc_1m")
    g.add_argument("--watchlist-verde", action="store_true", help="Tickers VERDE da watchlist")
    p.add_argument("--intervals", required=True, help="CSV de minutos: 5,15,30,60")
    p.add_argument(
        "--since", default="2020-01-01", help="Data minima (YYYY-MM-DD); default 2020-01-01"
    )
...
def main() -> int:
    ...
    for i in args.intervals.split(","):
        ...
        try:
            n = int(i)
            if n <= 0 or n > 1440:
                print(f"intervalo invalido (1-1440): {i}", file=sys.stderr)
                return 2
            intervals.append(n)
        except ValueError:
            print(f"intervalo nao-numerico: {i}", file=sys.stderr)
            return 2

    try:
        since_date = date.fromisoformat(args.since)
    except ValueError:
        print(f"--since invalido (use YYYY-MM-DD): {args.since}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if not tickers:
            print("nenhum ticker selecionado", file=sys.stderr)
            return 2
        print(
            f"resample: {len(tickers)} tickers x {len(intervals)} intervals "
                ...
                try:
                    with conn.cursor() as cur:
                        if args.dry_run:
                            ...
                            cur.execute(_RESAMPLE_SQL, (n_min, n_min, ticker, since_date))
                            n_rows = cur.rowcount
                            conn.commit()
                    totals[n_min] += n_rows
                    elapsed_t = _time.time() - t_t
                    print(
                        f"  [{i}/{len(tickers)}] {ticker:<7} {n_min:>3}m: "
