...
def load_rows(ticker: str, include_rf: bool, horizon: int) -> list[Row]:
    ...
    for i, r in enumerate(raw):
        ...
        tgt = None
        if (i + horizon < len(raw) and close is not None
                and closes[i + horizon] and closes[i + horizon] > 0 and close > 0):
...
def predict_q(models: dict, feats_vec: list[float]) -> tuple[float, float, float]:
    ...
    return (float(models[0.10].predict(x)[0]),
...
class WalkForwardStrategy:
    ...
    def __init__(self, features: list[str], rows: list[Row],
                 th_buy: float, th_sell: float,
                 test_start_idx: int, retrain_days: int):
...
def rows_to_bars(rows: list[Row], start_idx: int) -> list[dict[str, Any]]:
    ...
    for i in range(start_idx, len(rows)):
        ...
        bars.append({"time": int(dt.timestamp()),
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--retrain-days", type=int, default=63, help="re-treino a cada N dias úteis no test")
...
def main() -> int:
    ...
    print(f"WF backtest {args.ticker}  feat={len(features)}  h={args.horizon}d  retrain={args.retrain_days}d  include_rf={not args.no_rf}")
    ...
    if test_start_idx >= len(rows) - 20:
        ...
        return 2
    print(f"  total rows={len(rows)}  test_start_idx={test_start_idx}  test_len={len(rows) - test_start_idx}")

    strategy = WalkForwardStrategy(
        features=features, rows=rows,
        th_buy=args.th_buy, th_sell=args.th_sell,
    ...
    result = run_backtest(
        ...
        commission_pct=args.commission, range_period="test_wf",
    )
    ...
    if strategy.scores_log:
        ...
        print(f"Scores: mean={arr.mean():.4f}  std={arr.std():.4f}  "
