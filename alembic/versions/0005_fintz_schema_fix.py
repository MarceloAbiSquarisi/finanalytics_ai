...
def upgrade() -> None:
    ...
    op.add_column(
        "fintz_cotacoes", sa.Column("quantidade_negociada", sa.BigInteger(), nullable=True)
    )
    op.add_column(
        "fintz_cotacoes", sa.Column("quantidade_negocios", sa.BigInteger(), nullable=True)
    )
    op.add_column(
        "fintz_cotacoes",
        sa.Column("fator_ajuste_desdobramentos", sa.Numeric(18, 10), nullable=True),
    )
    op.add_column(
        "fintz_cotacoes",
        sa.Column("preco_fechamento_ajustado_desdobramentos", sa.Numeric(18, 6), nullable=True),
    )
    ...
    op.create_index(
        "ix_fintz_itens_item_periodo", "fintz_itens_contabeis", ["item", "tipo_periodo"]
    )
