...
def make_cotacoes_df() -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {
            "ticker": "PETR4", "data": date(2025, 1, 2),
            "preco_abertura": 34.0, "preco_fechamento": 35.0,
            "preco_maximo": 35.5, "preco_medio": 34.8, "preco_minimo": 33.5,
            "volume_negociado": 1000000.0, "quantidade_negociada": 28571,
            "quantidade_negocios": 5000, "fator_ajuste": 1.0,
            "preco_fechamento_ajustado": 35.0,
            "fator_ajuste_desdobramentos": 1.0,
            "preco_fechamento_ajustado_desdobramentos": 35.0,
        },
        {
            "ticker": "PETR4", "data": date(2025, 1, 3),
            "preco_abertura": 35.0, "preco_fechamento": 36.0,
            "preco_maximo": 36.5, "preco_medio": 35.8, "preco_minimo": 34.5,
            "volume_negociado": 900000.0, "quantidade_negociada": 25000,
            "quantidade_negocios": 4500, "fator_ajuste": 1.0,
...
def make_itens_df() -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {"ticker": "PETR4", "item": "ReceitaLiquida",
         "tipo_periodo": "12M", "data_publicacao": date(2025, 3, 31), "valor": 500e9},
        {"ticker": "PETR4", "item": "LucroLiquido",
         "tipo_periodo": "12M", "data_publicacao": date(2025, 3, 31), "valor": 100e9},
    ])
...
def make_indicadores_df() -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {"ticker": "PETR4", "indicador": "P/L",
         "data_publicacao": date(2025, 1, 2), "valor": 8.5},
        {"ticker": "PETR4", "indicador": "ROE",
         "data_publicacao": date(2025, 1, 2), "valor": 0.25},
    ])
...
class TestHelpers:
    def test_to_utc_datetime_from_date(self):
        from finanalytics_ai.infrastructure.database.repositories.timescale_writer import _to_utc_datetime
        d = date(2025, 1, 15)
    ...
    def test_nan_to_none_nan(self):
        import math
        from finanalytics_ai.infrastructure.database.repositories.timescale_writer import _nan_to_none
...
class TestPgTimescaleWriterItens:
    ...
    async def test_write_itens_uses_data_publicacao(self):
        ...
        assert records[0][0].tzinfo == timezone.utc
...
class TestNoOpTimescaleWriter:
    ...
    async def test_all_return_zero(self):
        from finanalytics_ai.infrastructure.database.repositories.timescale_writer import NoOpTimescaleWriter
